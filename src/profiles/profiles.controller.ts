...
export class ProfilesController {
  ...
  followProfile(@Param('username') username: string, @RequiredUser() user: AuthenticatedUser) {
    return this.profilesService.followProfile(username, user.id);
  }

  @Delete(':username/follow')
  @UseGuards(AuthTokenGuard)
  unfollowProfile(@Param('username') username: string, @RequiredUser() user: AuthenticatedUser) {
